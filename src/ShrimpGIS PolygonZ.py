...
"""
Construct a ShrimpGIS PolygonZ. Use it if you have z values.
    Args:
...
ghenv.Component.Name = "ShrimpGIS PolygonZ"
ghenv.Component.NickName = "shrimp_polygonz"
...
try:
    ...
    from shrimp_gis.geometry import ShpPolygonZ
...
def main():
    ...
    if _surface:
        shp_polygon = [ShpPolygonZ(srf, num) for srf in _surface]
        for geo in shp_polygon:
            geo.coordinates = []
            for pts in geo.points:
                geo.coordinates.append(get_latlon_from_location(pts, location, True))
